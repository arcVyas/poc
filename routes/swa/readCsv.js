...
readCsv.readCSVAndLoad= function(fileList, callback){
  _fileList = fileList
...
function writeToMongo(doc,callback){
...
function writeMetaToMongo(doc,callback){
  var db = mongoUtil.getDb();
  var collection = db.collection("ais_metrics_report_meta");
  doc["insertTime"]= moment().format('YYYY-MM-DD HH:mm:ss.SSS')
  collection.insert(doc, function(err, result) {
